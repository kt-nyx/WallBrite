...
namespace WallBrite
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void RefreshImageGrid()
        {
            ...
            if (WBLibrary.SortOrder == "descending")
            {
                for (int i = 0; i < WBLibrary.LibraryList.Count; i++)
                {
                    newGridInfo = DrawGridImage(i, availableGridWidth, row, column, totalWidth, firstImage);
                    row = newGridInfo.newRow;
                    column = newGridInfo.newColumn;
                    totalWidth = newGridInfo.newWidth;
                    firstImage = newGridInfo.stillFirst;
                }
            }
            else
            {
                for (int i = WBLibrary.LibraryList.Count - 1; i > 0; i--)
                {
                    newGridInfo = DrawGridImage(i, availableGridWidth, row, column, totalWidth, firstImage);
                    row = newGridInfo.newRow;
                    column = newGridInfo.newColumn;
        ...
        private dynamic DrawGridImage(int imageIndex, double availableGridWidth, int row, int column, int totalWidth, bool firstImage)
        {
            ...
            return new { newRow = row, newColumn = column, newWidth = totalWidth, stillFirst = firstImage };
        }
    }
}